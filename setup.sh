...
phase_env() {
  ...
  "GITHUB_API_BASE|GitHub API base URL|GitHub REST API base URL for repository administration and pull-request operations.|Use https://api.github.com for GitHub.com or the GHES API root for enterprise installations."
  "GITHUB_TOKEN|GitHub token|Bearer token with minimal repository administration, PR, issue, and metadata permissions.|Provisioned as a GitHub App installation token or fine-grained personal access token."
  )
...
phase_db() {
  ...
  if [[ -n "${PG_CONNECTION_STRING:-}" ]]; then
    ...
    if ${local_pg}; then
      ...
      warn "     Then set in .env: PG_CONNECTION_STRING=postgresql://postgres:secret@localhost:5432/repsso"
      exit 1
    fi
